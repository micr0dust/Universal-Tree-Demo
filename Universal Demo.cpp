...
// Universal Demo.cpp: 定義應用程式的類別表現方式。
//
...
BEGIN_MESSAGE_MAP(CUniversalDemoApp, CWinApp)
	ON_COMMAND(ID_APP_ABOUT, &CUniversalDemoApp::OnAppAbout)
	// 依據文件命令的標準檔案
...
// CUniversalDemoApp 建構

CUniversalDemoApp::CUniversalDemoApp() noexcept
{
	...
	SetAppID(_T("UniversalDemo.AppID.NoVersion"));

	// TODO:  在此加入建構程式碼，
	// 將所有重要的初始設定加入 InitInstance 中
}

// 唯一一個 CUniversalDemoApp 物件

CUniversalDemoApp theApp;


// CUniversalDemoApp 初始化

BOOL CUniversalDemoApp::InitInstance()
{
	...
	pDocTemplate = new CSingleDocTemplate(
		IDR_MAINFRAME,
		RUNTIME_CLASS(CUniversalDemoDoc),
		RUNTIME_CLASS(CMainFrame),       // 主 SDI 框架視窗
		RUNTIME_CLASS(CUniversalDemoView));
	if (!pDocTemplate)
		return FALSE;
...
int CUniversalDemoApp::ExitInstance()
{
	// TODO: 處理其他您已經加入的資源
	AfxOleTerm(FALSE);
...
// 執行對話方塊的應用程式命令
void CUniversalDemoApp::OnAppAbout()
{
	CAboutDlg aboutDlg;
	aboutDlg.DoModal();
}
